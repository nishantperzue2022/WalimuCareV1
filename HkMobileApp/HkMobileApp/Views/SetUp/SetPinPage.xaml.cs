...
namespace HkMobileApp.Views
{
    ...
    public partial class SetPinPage : ContentPage
    {
                    ...
                    if (response.IsSuccessful)
                    {
                        ...
                        if (deserializedResponse.success)
                        {
                            ...
                            if (!IsChangePasswordRequest)
                            {
                                ...
                                Preferences.Set(nameof(AspNetUsers.firstName), userData.VerifyMember.data.bls_member.first_name);
                                Preferences.Set(nameof(AspNetUsers.lastName), userData.VerifyMember.data.bls_member.last_name);
                                Preferences.Set(nameof(AspNetUsers.email), userData.VerifyMember.data.bls_member.email);
                                Preferences.Set(nameof(AspNetUsers.phoneNumber), userData.VerifyMember.data.bls_member.mobile_phone_number);
                                Preferences.Set("dateofbirth", userData.VerifyMember.data.bls_member.date_of_birth);
                                Preferences.Set("gender", userData.VerifyMember.data.bls_member.gender);
        ...
        public async void btnSetPin_Clicked(object sender, EventArgs e)
        {
            try
            {
                await Navigation.PushPopupAsync(new HkLoaderPage("Please wait as we set up your pin"));
